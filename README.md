...
Audio scripts, dependency links, Japanese token states, timing, transcripts, questions, explanations, source references, and TTS records are versioned. The current 60-lesson authored catalog uses measured 11–15 minute scripts with at least 95% Japanese speech and eight listening checks per lesson. Draft validation audits exact and near-duplicate heard content across all audio lessons and blocks normal production publishing until links are verified and every section has a ready hosted audio file. System speech is only a local-development review fallback. See [Audio Lessons](docs/audio-lessons.md) for setup, seeding, preview refresh, management routes, TTS environment variables, and production limitations.
