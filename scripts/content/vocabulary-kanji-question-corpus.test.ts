...
describe("canonical vocabulary and kanji learning question corpus", () => {
  it("parses the shared Phase 1 schema with deterministic unique IDs", () => {
    ...
    expect(content.questions.filter(({ domain }) => domain === "vocabulary")).toHaveLength(6912);
  ...
  it("gives every release vocabulary six questions with sense and reading coverage", () => {
    ...
    expect(releaseTargets).toHaveLength(1152);
  ...
  it("calculates inventory-bounded kanji targets without inventing vocabulary", () => {
    ...
    expect(releaseTargets).toHaveLength(223);
    expect(releaseTargets.filter(({ inventoryLimitation }) => inventoryLimitation)).toHaveLength(48);
    expect(releaseTargets.every((row) =>
      row.approvedQuestionIds.length === row.effectiveSupportedCount && row.status === "pass",
    )).toBe(true);
  });
  ...
  it("reuses existing sentences and preserves Phase 2 and Phase 4 IDs", () => {
    ...
    expect(content.sentences).toHaveLength(n5Sentences.sentences.length + n4Sentences.sentences.length);
