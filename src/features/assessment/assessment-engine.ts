...
export class AssessmentEngine {
  ...
  private selectKnowledge(candidates: Candidate[], domain: Domain, count: number, level: 'N5' | 'N4', targetLimit: number, selectedIds: Set<string>, relaxations: AssessmentSnapshot['relaxedConstraints']): Candidate[] {
    const domainCandidates = candidates.filter((candidate) => candidate.domain === domain && !candidate.parentId && !selectedIds.has(candidate.question.id)); const chosen: Candidate[] = []; const targetCounts = new Map<string, number>(); const desiredN4 = level === 'N4' ? Math.round(count * 0.75) : 0;
