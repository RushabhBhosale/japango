...
export default function ContentResultsScreen() {
  ...
  return <ScreenContainer contentStyle={styles.centered}><ThemedText type="smallBold" themeColor="primary">SESSION COMPLETE</ThemedText><ThemedText type="title">Nice focused work.</ThemedText><Card><View style={styles.row}><ThemedText style={styles.score}>{result.percentage}%</ThemedText><ThemedText type="heading">{result.correctCount} / {result.totalQuestions}</ThemedText></View><ProgressBar value={result.percentage} accessibilityLabel="Lesson score" /><ThemedText themeColor="textSecondary">{result.incorrectCount} incorrect · {Math.floor(result.timeTakenSeconds / 60)} min {result.timeTakenSeconds % 60}s</ThemedText><ThemedText type="smallBold">Recommended next action: {recommendation}</ThemedText><ThemedText themeColor="textSecondary">Lesson mastery needs repeated success across quiz and review sessions.</ThemedText></Card><AppButton label="Continue learning" onPress={() => router.replace('/(tabs)/learn')} /><AppButton label="View progress" variant="secondary" onPress={() => router.replace('/(tabs)/progress')} /></ScreenContainer>;
}
