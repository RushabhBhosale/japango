...
export default function ContentPracticeScreen() {
  ...
  return <ScreenContainer>
    ...
    <ProgressBar value={progress} accessibilityLabel="Lesson practice progress" /><Card><ThemedText type="smallBold" themeColor="primary">{question.presentation.replaceAll('-', ' ').toUpperCase()}</ThemedText><ThemedText type="heading">{question.prompt}</ThemedText></Card>
    ...
    {attempt ? <Card style={{ backgroundColor: attempt.correct ? theme.successSoft : theme.errorSoft }}><ThemedText type="heading">{attempt.correct ? 'Correct' : 'Not quite'}</ThemedText><ThemedText>{question.explanation ?? 'The correct answer is highlighted above.'}</ThemedText></Card> : null}
